...
def handle_client(client_socket, client_address):
    ...
    try:
        ...
        # Tạo khóa AES cho phiên làm việc
        aes_key = get_random_bytes(16)
        ...
        while True:
            try:
                ...
                if not encrypted_message:
                    break  # Nếu client đóng kết nối, thoát vòng lặp

                decrypted_message = decrypt_message(aes_key, encrypted_message)
                print(f"Received from {client_address}: {decrypted_message}")

                if decrypted_message == "exit":
                    break  # Thoát vòng lặp nếu client gửi "exit"
                ...
                break  # Nếu client mất kết nối đột ngột, thoát vòng lặp

    except Exception as e:
        print(f"Error handling client {client_address}: {e}")

    finally:
        # Đảm bảo xóa client khỏi danh sách và đóng kết nối
        clients.remove((client_socket, aes_key))
        client_socket.close()
        print(f"Connection with {client_address} closed")
